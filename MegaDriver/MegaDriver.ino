...
double kpLineSensor = 50;            //Proportional Gain for Line Following
...
int16_t sensor_bias[8] = { 100, 152, 208, 100, 100, 100, 152, 152 };  // sensor biases
...
double baseDist = 14 * 2.54;

double baseAngle = M_PI / 2;
...
void LineFollow(void) {
  ...
  for (uint8_t i = 0; i < 8; i++) {
    num += abs(Sensor_value_unbiased[i]) * sensor_loc[i];
    denom += abs(Sensor_value_unbiased[i]);
  }
...
void CheckIntersection(void) {
  ...
  if (avg > 1200) {
    ...
    if (currIntersection != activeIntersection) {
      ...
      Turn(-M_PI / 14);
      delay(750);
      Straight(baseDist * 1.1);
      delay(750);
      Turn(baseAngle * 0.95);
      delay(750);
      Straight(baseDist * 1.55);
      Reset();
      state = 2;
    }
  } else {
...
void Straight(double dist) {
  ...
  // Control parameters
  double Kp = 2;  // Proportional Gain for Trajectory Following
  double Kdp = 0.5;
  ...
  while (true) {
    ...
    counts1 = abs(myEnc1.read());
    counts2 = abs(myEnc2.read());
    ...
    double error1 = theta1_des - theta1;
    double error2 = theta2_des - theta2;

    omega1 = error1 / deltaT;
    omega2 = error2 / deltaT;
    ...
    // Compute the control signal (proportional control)
    V1m = Kp * error1 + Kdp * (error1 - oldError1) / deltaT;
    V2m = Kp * error2 + Kdp * (error2 - oldError2) / deltaT;

    // Stop motors if target distance is reached
    if (abs(theta1) >= abs(theta1_final) || abs(theta2) >= abs(theta2_final)) {
      myEnc1.readAndReset();
      myEnc2.readAndReset();
      mdWheels.setSpeeds(0, 0);
...
void CheckWall(void) {
  ...
  // } else if (analogRead(distSensor) < 50) {
  //   approach = true;
  // }
}
void FindRefinery(void) {
  ...
  // if the reading is greater than 180, stop
  if (analogRead(distSensor) > 75 && approach) {
    // stop the robot
    mdWheels.setSpeeds(0, 0);
    state = 6;
  ...
  else if (analogRead(distSensor) > 30 && analogRead(distSensor) < 50) {
    approach = true;
  }
}
